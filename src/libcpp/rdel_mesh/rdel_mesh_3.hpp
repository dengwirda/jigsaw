...
/*------------------------------------------ collar lists */
    ...
    __static_call
    __normal_call void_type lock_face (
        mesh_type &_mesh,
        typename
        ...
        )
    {
        ...
        __unreferenced( _opts ) ;
        for (auto _iter  =
            _mesh._fset._lptr.head () ;
                  _iter !=
            _mesh._fset._lptr.tend () ;
          ++_iter  )
        {
        //  protect all restricted faces from refinement
            for (auto _item  = *_iter ;
                _item != nullptr;
                _item  = _item->_next )
            {
            face_data _fdat(_item->_data) ;
            algorithms::isort (
                _fdat._node.head(),
                _fdat._node.tend(),
                std::less<iptr_type>()) ;
        ...
        )
    {
        ...
        real_list _npwr(_mesh._tria._nset.count(), 
            std::numeric_limits<real_type>::infinity());
        
        iptr_list _topo(_mesh._tria._nset.count(), +0) ;
                ...
                ++_iter  )
        {
            ...
            for (auto _item  = *_iter ;
                ...
                _item  = _item->_next )
            {
             auto _feat = _item->_data._feat ;
            if (_feat == mesh::hard_feat ||
                _feat == mesh::soft_feat ||
                _feat == mesh::user_feat)
            {
            ...
            _npwr[_inod] = 
                std::min(_lsqr, _npwr[_inod]) ;
            _topo[_inod] += 1 ;

            _npwr[_jnod] = 
                std::min(_lsqr, _npwr[_jnod]) ;
            _topo[_jnod] += 1 ;
            }
            }
        }
                ...
                ++_iter, ++_npos)
        {
            ...
            {
             real_type _ppos[4] = {
            _mesh._tria.node(_npos)->pval(0),
            _mesh._tria.node(_npos)->pval(1),
            _mesh._tria.node(_npos)->pval(2),
                 _npwr[_npos] } ;
            
            _mesh._tria.update(_ppos, _npos);
            }
        }
...
"#-----------------------------------------------------------------------\n"
    ...
    /*-------------------- push refinement scheme metrics */
        ...
        _dump.push(std::to_string(_tcpu._face_rule));
        ...
        _dump.push(
        std::to_string(_fnod[rdel_opts::circ_kind]));
        _dump.push("\n")  ;
        _dump.push(" *face-sink = ") ;
        _dump.push(
        std::to_string(_fnod[rdel_opts::sink_kind]));
        _dump.push("\n")  ;
        _dump.push(" *face-offH = ") ;
        _dump.push(
        std::to_string(_fnod[rdel_opts::offH_kind]));
        _dump.push("\n")  ;
        _dump.push(" *face-offC = ") ;
        _dump.push(
        std::to_string(_fnod[rdel_opts::offC_kind]));
        _dump.push("\n")  ;
        _dump.push(" *face-offT = ") ;
        _dump.push(
        std::to_string(_fnod[rdel_opts::offT_kind]));
        _dump.push("\n")  ;
    ...
    /*-------------------- more refinement scheme metrics */
        ...
        _dump.push(std::to_string(sizeof(face_cost)));
        _dump.push("\n")  ;
        _dump.push(" *fset-peak = ") ;
        ...
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::ORIENT3D_f])) ;
        _dump.push("\n")  ;
        _dump.push(" *orient3Di = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::ORIENT3D_i])) ;
        _dump.push("\n")  ;
        _dump.push(" *orient3De = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::ORIENT3D_e])) ;
        _dump.push("\n")  ;

        _dump.push(" *bisect3Df = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::BISECT3D_f])) ;
        _dump.push("\n")  ;
        _dump.push(" *bisect3Di = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::BISECT3D_i])) ;
        _dump.push("\n")  ;
        _dump.push(" *bisect3De = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::BISECT3D_e])) ;
        _dump.push("\n")  ;
        _dump.push(" *bisect3Wf = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::BISECT3W_f])) ;
        _dump.push("\n")  ;
        _dump.push(" *bisect3Wi = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::BISECT3W_i])) ;
        _dump.push("\n")  ;
        _dump.push(" *bisect3We = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::BISECT3W_e])) ;
        _dump.push("\n")  ;

        _dump.push(" *inball3Df = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::INBALL3D_f])) ;
        _dump.push("\n")  ;
        _dump.push(" *inball3Di = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::INBALL3D_i])) ;
        _dump.push("\n")  ;
        _dump.push(" *inball3De = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::INBALL3D_e])) ;
        _dump.push("\n")  ;
        _dump.push(" *inball3Wf = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::INBALL3W_f])) ;
        _dump.push("\n")  ;
        _dump.push(" *inball3Wi = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::INBALL3W_i])) ;
        _dump.push("\n")  ;
        _dump.push(" *inball3We = ") ;
        _dump.push(std::to_string(
         geompred::_nn_calls[geompred::INBALL3W_e])) ;
        _dump.push("\n")  ;
